            ...
            return {
                ...
                link: function ($scope, element, attrs, controller) {
                    ...
                    (function init() {
                        ...
                        $scope.predictions = [];
                        $scope.input = element;
                        $scope.options = $scope.options || { componentRestrictions: { country: 'us' },
                          types: ['geocode']};
                    ...
                    function select() {
                        ...
                        if (prediction.is_custom) {
                          if(prediction.display_custom_prediction_label_on_select && prediction.custom_prediction_label) {
                            $scope.model = prediction.description + " " + prediction.custom_prediction_label;
                          }
                          else {
                            ...
                            placesService.getDetails({ placeId: prediction.place_id }, function (place, status) {
                              $scope.model = prediction.description;
                                if (status == google.maps.places.PlacesServiceStatus.OK) {
                                    $scope.$apply(function () {
                                        //$scope.model = place.formatted_address;
                        ...
                        for (i = 0; i < $scope.customPlaces.length; i++) {
                            ...
                            if (match.matched_substrings.length > 0) {
                                predictions.push({
                                    ...
                                    terms: match.terms,
                                    display_custom_prediction_label_on_select: place.display_custom_prediction_label_on_select || false
                                });
                            }
                        }
