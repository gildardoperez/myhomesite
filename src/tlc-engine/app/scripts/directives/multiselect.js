        ...
        function getHeaderText() {
            ...
            if (isMultiple) {
                if (attrs.msSelected) {
                  ...
                  if (header.length == 0 || header.length == scope.items.length) {
                    header = [];
                    header.push('All');
                  }
                  scope.header = header.join(', ');
                }
            } else {
...
.directive('amMultiselectPopup', ['$document', function ($document) {
    return {
        ...
        templateUrl: function (element, attr) {
            return attr.templateUrl || 'views/templates/multiselect.html';
        },
        link: function (scope, element, attrs) {
            ...
            scope.verifyCheckAll = function()
            {
              ...
              return scope.items.length != i;
            }

            scope.verifyUnCheckAll = function()
            {
              var i = 0;
              angular.forEach(scope.items, function (item) {
                if(item.checked)
